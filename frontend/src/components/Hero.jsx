...
const Hero = () => {
    ...
    const devPrompt = `${portfolioConfig.terminalProfile?.user || 'aman@dev'}:~$ `;
    const [currentPrompt, setCurrentPrompt] = useState(devPrompt);
    ...
    useEffect(() => {
        let isMounted = true;
        const devUser = portfolioConfig.terminalProfile?.user || 'aman@dev';

        const steps = [
            { prompt: `${devUser}:~$ `, cmd: 'cd frontend', output: null, delayAfter: 600 },
            { prompt: `${devUser}:~/frontend$ `, cmd: 'npm run dev', output: '➜  Local: http://localhost:5173/  [Vite Ready ⚡]', delayAfter: 1200 },
            { prompt: `${devUser}:~$ `, cmd: 'cd server', output: null, delayAfter: 600 },
            { prompt: `${devUser}:~/server$ `, cmd: 'npm run dev', output: '⚡ Server running on port 5000 [MongoDB Connected 🚀]', delayAfter: 2000 },
        ];
    ...
    const copyCodeSnippet = () => {
        ...
        if (terminalTab === 'bash') {
            code = `cd frontend\nnpm run dev\ncd server\nnpm run dev`;
        } else if (terminalTab === 'profile.json') {
            code = JSON.stringify(portfolioConfig.terminalProfile || {
                name: portfolioConfig.name || 'Developer',
                role: portfolioConfig.role || 'Full-Stack Developer',
                    ...
                    <div className={styles.heroCardInner}>
                        ...
                        <div className={styles.heroContent}>
                            ...
                            <h1 className={styles.title}>
                                {portfolioConfig.name || 'Developer'} {portfolioConfig.lastName && <span className={styles.highlight}>{portfolioConfig.lastName}</span>}
                            </h1>
                            ...
                            <p className={styles.description}>
                                {portfolioConfig.description || portfolioConfig.tagline || 'code — deploy — repeat'}
                            </p>
                            ...
                            <div className={styles.socialLinks}>
                                ...
                                {/* Clean default icons if none configured yet */}
                                {!hasAnySocial && (
                                    <>
                            ...
                            <div className={styles.ctaButtons}>
                                ...
                                <a
                                    href={portfolioConfig.resumeUrl || '/resume.pdf'}
                                    download="Resume.pdf"
                            ...
                            <div className={styles.codeTerminal}>
                                <div className={styles.terminalHeader}>
                                    ...
                                    <div className={styles.terminalTabs}>
                                        ...
                                        <button
                                            className={`${styles.tabBtn} ${terminalTab === 'profile.json' ? styles.tabActive : ''}`}
                                            onClick={() => setTerminalTab('profile.json')}
                                        >
                                            profile.json
                                        </button>
                                        <button
                                ...
                                <div className={styles.terminalBody}>
                                            ...
                                            {typedLines.map((line, idx) => (
                                                <div key={idx} className={styles.consoleLineGroup}>
                                                    <div className={styles.consolePromptLine}>
                                                        <span className={styles.promptUser}>{portfolioConfig.terminalProfile?.user || 'aman@dev'}</span>
                                    ...
                                    {terminalTab === 'profile.json' && (
                                        <pre className={styles.codeBlock}>
                                            <code>
<span className={styles.tokenPunctuation}>&#123;</span>{'\n'}
{'  '}<span className={styles.tokenKey}>"name"</span><span className={styles.tokenPunctuation}>:</span> <span className={styles.tokenString}>"{portfolioConfig.terminalProfile?.name || portfolioConfig.name || 'Developer'}"</span><span className={styles.tokenPunctuation}>,</span>{'\n'}
{'  '}<span className={styles.tokenKey}>"role"</span><span className={styles.tokenPunctuation}>:</span> <span className={styles.tokenString}>"{portfolioConfig.terminalProfile?.role || 'Full-Stack Developer'}"</span><span className={styles.tokenPunctuation}>,</span>{'\n'}
{'  '}<span className={styles.tokenKey}>"status"</span><span className={styles.tokenPunctuation}>:</span> <span className={styles.tokenString}>"{portfolioConfig.terminalProfile?.status || 'Building & Deploying 🚀'}"</span><span className={styles.tokenPunctuation}>,</span>{'\n'}
{'  '}<span className={styles.tokenKey}>"openToWork"</span><span className={styles.tokenPunctuation}>:</span> <span className={styles.tokenBoolean}>{portfolioConfig.terminalProfile?.openToWork !== false ? 'true' : 'false'}</span>{'\n'}
<span className={styles.tokenPunctuation}>&#125;</span>
                                            </code>
                                        </pre>
                                    )}
...
{'  '}<span className={styles.tokenKey}>frontend</span><span className={styles.tokenPunctuation}>:</span> <span className={styles.tokenPunctuation}>[</span><span className={styles.tokenString}>"React"</span><span className={styles.tokenPunctuation}>,</span> <span className={styles.tokenString}>"JavaScript"</span><span className={styles.tokenPunctuation}>,</span> <span className={styles.tokenString}>"Tailwind"</span><span className={styles.tokenPunctuation}>]</span><span className={styles.tokenPunctuation}>,</span>{'\n'}
...
{'  '}<span className={styles.tokenKey}>languages</span><span className={styles.tokenPunctuation}>:</span> <span className={styles.tokenPunctuation}>[</span><span className={styles.tokenString}>"JavaScript"</span><span className={styles.tokenPunctuation}>,</span> <span className={styles.tokenString}>"Python"</span><span className={styles.tokenPunctuation}>,</span> <span className={styles.tokenString}>"TypeScript"</span><span className={styles.tokenPunctuation}>]</span><span className={styles.tokenPunctuation}>,</span>{'\n'}
